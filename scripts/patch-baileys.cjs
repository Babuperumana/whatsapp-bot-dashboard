/**
 * patch-baileys.cjs
 * Fixes make-in-memory-store.js in @kelvdra/baileys which uses CJS require()
 * inside an ESM context. Runs automatically via "postinstall" in package.json.
 */
const fs = require('fs')
const path = require('path')

const target = path.join(__dirname, '..', 'node_modules', '@kelvdra', 'baileys', 'lib', 'Store', 'make-in-memory-store.js')

if (!fs.existsSync(target)) {
  console.log('patch-baileys: target file not found, skipping.')
  process.exit(0)
}
...
// Already patched?
if (original.includes('KeyedDBModule')) {
  console.log('patch-baileys: already patched, skipping.')
  process.exit(0)
}

const patched = `import WAProto from '../../WAProto/index.js';
...
import WAProto_1_default from '../../WAProto/index.js';
const WAProto_1 = WAProto_1_default;
...
import * as repository_1 from './object-repository.js';
import KeyedDBModule from '@adiwajshing/keyed-db';
const _KeyedDB = KeyedDBModule.default || KeyedDBModule;
...
const makeInMemoryStore = (config) => {
    ...
    return {
        ...
        fromJSON,
        writeToFile: (path) => {
            import('fs').then(({ writeFileSync }) => writeFileSync(path, JSON.stringify(toJSON())))
        },
        readFromFile: (path) => {
...
console.log('patch-baileys: patched make-in-memory-store.js successfully.')
